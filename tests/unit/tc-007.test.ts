...
describe('TC-007 S3 auth failure (403) writes error run record', () => {
  ...
  it('TC-007: records ingestion_run with status=error and AccessDenied in error_message when S3 returns 403', async () => {
    const accessDeniedError = new Error('AccessDenied: Access Denied');
    (accessDeniedError as any).name = 'AccessDenied';
    (accessDeniedError as any).$metadata = { httpStatusCode: 403 };

    const sendMock = vi.fn().mockRejectedValue(accessDeniedError);
    ...
    expect(runMutationMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        rows_inserted: 0,
        rows_skipped: 0,
        error_message: expect.stringContaining('AccessDenied'),
      })
    );
  ...
  it('TC-007: does not throw when S3 returns 403 AccessDenied (polling continues)', async () => {
    const accessDeniedError = new Error('AccessDenied: Access Denied');
    (accessDeniedError as any).name = 'AccessDenied';
    (accessDeniedError as any).$metadata = { httpStatusCode: 403 };

    vi.mocked(S3Client).mockImplementation(function(this: any) {
      this.send = vi.fn().mockRejectedValue(accessDeniedError);
    } as any);
